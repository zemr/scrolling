function detectMobileChrome() {
  if (!!window.chrome) {
    var chromeV = /Chrome\/([0-9.]+)/.exec(navigator.userAgent)[1];
    return chromeV.split(".")[0] > 33;
  } else {
    return false;
  }
}

if (!Modernizr.touch || detectMobileChrome()) {
  var controller = new ScrollMagic.Controller();

  new TimelineMax()
    .to('.diver', 3, {bottom: 96, ease: Power0.easeOut})
    .fromTo('.diver', 2.5, {opacity: .1}, {opacity: 1, ease: Power1.easeIn}, .5)
  ...
  var head = document.getElementById('fishhead');
